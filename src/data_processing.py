...
def get_val_data(valA, C_dist, exper, nx):
    path = f"../mop_modifications/streamlined_mop/outputs/GPT2/{exper}/data/val_{valA}{C_dist}_state_dim_{nx}.pkl"
    obs = []
...
def get_other_err(valA, C_dist, ckpt_step, exper, curve, nx):
    path = f"../mop_modifications/streamlined_mop/outputs/GPT2/{exper}/prediction_errors{C_dist}_step={str(ckpt_step)}.ckpt/{valA}_state_dim_{nx}_err_lss.pkl"
    curve_err = None
