...
class Config(object, metaclass=Singleton):
    ...
    val_dataset_typ = "upperTriA" #"unifA" #"gaussA" #"gaussA_noscale" #"rotDiagA" #"rotDiagA_unif" #"rotDiagA_gauss" #"upperTriA" #"single_system" #"cond_num"
    ...
    # Training settings
    devices=1 #which GPU
    train_steps = 5530 #number of training steps (num_tasks*num_traces[train])/batch_size
    num_epochs = 1000
    train_int = 5 #number of steps between logging (train interval)
    use_true_len = True
    batch_size = 512 #does changing this by 2x or 0.5x change the training time? (if no effect than batch size is too big)
    train_data_workers = 254 #set to 1 (check if it changes the speed of the training process)
    test_batch_size = 512 #256
    test_data_workers = 254 #4 #set to 1
